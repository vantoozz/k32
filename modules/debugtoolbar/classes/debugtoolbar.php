...
class DebugToolbar
{
	...
	public static function render($print = false)
	{
		...
		// Database panel
		if (Kohana::$config->load('debug_toolbar.panels.database') === TRUE)
		{
		...
		// Files panel
		if (Kohana::$config->load('debug_toolbar.panels.files') === TRUE)
		{
			$template->set('files', self::get_files());
		}

		// Modules panel
		if (Kohana::$config->load('debug_toolbar.panels.modules') === TRUE)
		{
			$template->set('modules', self::get_modules());
		}

		// Routes panel
		if (Kohana::$config->load('debug_toolbar.panels.routes') === TRUE)
		{
			$template->set('routes', self::get_routes());
		}

		// Custom data
		if (Kohana::$config->load('debug_toolbar.panels.customs') === TRUE)
		{
		...
		switch (Kohana::$config->load('debug_toolbar.align'))
		{
			case 'right':
			case 'center':
			case 'left':
				$template->set('align', Kohana::$config->load('debug_toolbar.align'));
				break;
			default:
		...
		// Benchmarks panel
		if (Kohana::$config->load('debug_toolbar.panels.benchmarks') === TRUE)
		{
	...
	public static function is_enabled()
	{
		...
		$secret_key = Kohana::$config->load('debug_toolbar.secret_key');
